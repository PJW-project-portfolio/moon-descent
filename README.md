...
지형은 좌우 끝이 이어지는 무한 래핑 맵입니다. 맵 한 바퀴의 폭은 스테이지마다
400 px씩 줄어듭니다: `MOON 6,400 px`, `MARS 6,000 px`, `VENUS 5,600 px`
(화면 표시 기준 640 m / 600 m / 560 m).
...
점수를 기록할 때 **영문 이니셜 2~4자**와 **전화번호 뒤 4자리**를 입력합니다.
예: 홍길동, 010-1234-0412 → `HGD` `0412`

- 글자는 키 위치로 읽으므로 한/영이 한글 상태여도 영문으로 입력됩니다.
- 숫자는 윗줄 숫자키나 숫자패드로 입력합니다. 이니셜과 번호는 각자 칸에 들어가므로
  `HGD` 입력 후 바로 `0412`를 치고 `Enter`를 누르면 됩니다. 둘 중 하나라도 덜 채우면
  저장되지 않고 안내 문구가 빨갛게 바뀝니다.
- `Backspace`는 번호부터 지우고, 번호가 비면 이니셜을 지웁니다. `Esc`는 기록하지
  않고 넘어갑니다.
...
- 파일럿 ID 도입 전에 저장된 닉네임 기록은 이니셜 칸에 기존 닉네임, 번호 칸에 `----`로
  표시됩니다.
